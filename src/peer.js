...
export class Peer {
    constructor(peerIndex, download) {
        ...
        finished(this.socket, (error) => {
            ...
            if (this.currentPieceIndex !== null) {
                this.download.piecesToDownload.add({
                    priority: 0,
                    value: this.currentPieceIndex,
                });
    ...
    async parse(chunk) {
        ...
        if (!this.handshakeReceived) {
            ...
            if (this.reservedBits.has(0)) {
                this.socket.write(message.encodePort(Node.port));
            }
        ...
        if (messageID === message.ID_CHOKE) {
            ...
            console.log(this.download.available);
            ...
            try {
                const result = await this.download.node.getPeers(
                    port,
                    address,
                    this.download.torrent.infoHash()
                );
    ...
    #request() {
        if (this.currentPieceIndex === null) {
            ...
            this.buffer.fill(0);
            this.currentPieceIndex = this.download.piecesToDownload.pop().value;
        }

        if (
            this.available === null ||
            !this.available[this.currentPieceIndex]
        ) {
            this.socket.destroy();
